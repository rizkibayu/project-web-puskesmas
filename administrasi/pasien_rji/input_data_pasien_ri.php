  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title text-center">Tambah Data Pasien Rawat Inap</h4>
...
<form action="" method="POST" enctype="multipart/form-data">
	<div class="form-row">
		...
		<div class="form-group col-md-6">
			...
			<input type="date" class="form-control" name="tanggal_keluar" readonly="">
		</div>
	</div>
	<div>
			...
			<option value="">-- Pilih Kamar --</option>
				<?php 
					$ambil = $koneksi->query("SELECT * FROM kamar");
						...
						?>
						<option value="<?php echo $perkamar['id_kamar'] ?>">
					<?php echo $perkamar['id_kamar'] ?> - 
				<?php echo $perkamar['nama_kamar'] ?>
			</option>
		<?php } ?>
		</select>
	</div>				
	<div class="form-group">
			...
			<input type="number" class="form-control" name="jml_kasur">
	</div>
	<div class="form-group">
	<div class="form-group">
...
if (isset($_POST['input'])) {
	...
	$ambil = $koneksi->query("SELECT * FROM kamar WHERE id_kamar='$id_kamar'");
			$arraykamar = $ambil->fetch_assoc();
			$id_kamar = $arraykamar['id_kamar'];
			$nama_kamar = $arraykamar['nama_kamar'];

$koneksi->query("INSERT INTO rawat_inap(tanggal_masuk,nama_kamar,jml_kasur,id_kamar,id_rekam, id_obat, tanggal_rekam, nama_pasien, diagnosa, jenis_pemeriksaan, obat, jml_obat, pengguna, status) VALUES ('$tanggal_masuk','$nama_kamar','$jml_kasur','$id_kamar','$id_rekam','$id_obat','$tanggal_rekam','$nama_pasien','$diagnosa','$jenis_pemeriksaan','$obat','$jml_obat','$pengguna','Belum Keluar')");

	$koneksi->query("UPDATE obat SET stok_obat = stok_obat - $jml_obat WHERE id_obat = '$id_obat'");

	$koneksi->query("UPDATE kamar SET stok_kasur = stok_kasur - $jml_kasur WHERE id_kamar = '$id_kamar'");
