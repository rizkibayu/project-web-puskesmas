...
<form action="" method="POST" enctype="multipart/form-data">
	...
	<div class="form-group">
		<label for="">Dokter</label>
			<select name="nama_dokter" id="" class="form-control">
				...
				<?php while ($type = $ambil->fetch_assoc()) {?>
					<option><?php echo $type['nama_dokter'] ?></option>
				<?php }?>
			</select>
		</div> 
	<div class="form-group">
	...
	<br><?php if (isset($_POST['proses'])) {?>
		<div class="form-group col-md-4">
			...
			<input type="number" class="form-control" required="" name="qty_obat">
		</div>	
		<div class="form-group col-md-4">
			<select name="keterangan" id="" class="form-control">
				<option value="">--Pilih Keterangan--</option>
				<option value="1x1 Sehari">1x1 Sehari</option>
				<option value="2x1 Sehari">2x1 Sehari</option>
				<option value="3x1 Sehari">3x1 Sehari</option>
			</select>
		</div> 
		<div class="form-group col-md-4">
			<select name="tindakan" id="" class="form-control">
				<option value="">--Pilih Tindakan Rawat--</option>
...
if (isset($_POST['rekam'])) {
	...
	$koneksi->query("UPDATE rekam SET id_obat = '$id_obat',pasien = '$pasien',tanggal = '$tanggal',diagnosa = '$diagnosa',nama_dokter = '$nama_dokter',obat = '$nama_obat',status = 'Telah Di Periksa',keterangan = '$keterangan',poli = '$poli',pengguna = '$pengguna',tindakan = '$tindakan',qty_obat = '$qty_obat',pemeriksaan = '$pemeriksaan',status_pemberitahuan = 'Belum Dibayar' WHERE id_rekam = '$_GET[id]'");
